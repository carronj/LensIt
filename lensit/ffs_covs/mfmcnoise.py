...
class MFMCnoise_lib():
    ...
    def _get_rootPmat(self, _type, i, j):
        assert i < len(_type) and j < len(_type), (i, j, _type)
        if i > j: return self._get_rootPmat(_type, j, i)
        fname = self.lib_dir + '/Pmats/%s_rootPmat_%s%s.npy' % (_type, i, j)
        if not os.path.exists(fname):
            rPmat = SM.get_Pmat(_type, self.lib_alm, self.cls_len,
    ...
    def _get_rootPmatinv(self, _type, i, j):
        assert i < len(_type) and j < len(_type), (i, j, _type)
        if i > j: return self._get_rootPmatinv(_type, j, i)
        fname = self.lib_dir + '/Pmats/%s_rootPmatinv_%s%s.npy' % (_type, i, j)
        if not os.path.exists(fname):
            rPmat = SM.get_Pmat(_type, self.lib_alm, self.cls_len,
    ...
    def _get_Pmatinv(self, _type, i, j):
        assert i < len(_type) and j < len(_type), (i, j, _type)
        if i > j: return self._get_Pmatinv(_type, j, i)
        fname = self.lib_dir + '/Pmats/%s_Pmatinv_%s%s.npy' % (_type, i, j)
        if not os.path.exists(fname):
            rPmat = SM.get_Pmat(_type, self.lib_alm, self.cls_len,
    ...
    def _buildPmats(self, MFkey):
        ...
        if MFkey == 0:
            ...
            def get_W1(_type, i, j):
                return self._get_rootPmatinv(_type, i, j) * self.cl_transf[self.lib_alm.reduced_ellmat()]
    ...
    def eval(self, _type, MFkey, xlms, lib_qlm):
        """
        :param _type:
        :param MFkey:
        ...
        W1a, W1b = W1
        W2a, W2b = W2
        timer.checkpoint('Starting MF qest eval. %s MFkey %s' % (_type, MFkey))
        ...
        for i in range(len(_type)):
            ...
            for j in range(len(_type)):
                d1 += W1a(_type, i, j) * xlms[j]
                d2 += W2a(_type, i, j) * xlms[j]
            dy += _2alm(_2map(d1) * _2map(d2))
            d1 = np.zeros(self.lib_alm.alm_size, dtype=complex)
            d2 = np.zeros(self.lib_alm.alm_size, dtype=complex)
            for j in range(len(_type)):
                d1 += W1b(_type, i, j) * xlms[j]
                d2 += W2b(_type, i, j) * xlms[j]
    ...
    def evalMCnoise(self, _type, MFkey1, recache=False, MCnoise_floor=False):
        """
