...
Uses a NR iteration scheme, where the input displacement and magnification
...
FIXME : Retry to use templates.
TODO  : Pure potential, pure curl displacement inversion ? This would require 4 textures instead of 6
TODO  : Try non-texture based lensing ? (beware of boudaries however)
*/

#include <pycuda-complex.hpp>

texture<float, 2> tex_dx;  // dx displacement, in grid units
texture<float, 2> tex_dy;  // dy displacement, in grid units
texture<float, 2> Minv_xx; // negative inverse magnification, xx elements
texture<float, 2> Minv_xy; // negative inverse magnification, xy elements
texture<float, 2> Minv_yx; // negative inverse magnification, yx elements
texture<float, 2> Minv_yy; // negative inverse magnification, yy elements
...
// Just fetching textures
    ...
    ey[i] = tex2D(Minv_yx,_x,_y) * tex2D(tex_dx,_x,_y) + tex2D(Minv_yy,_x,_y) * tex2D(tex_dy,_x,_y) ;
    }
}
...
// For this we need the lens the 4 magnification matrix entries plus the two displacement components.
    ...
    float len_Myx =       cubicFilter(fy,
                     cubicFilter(fx, tex2D(Minv_yx, px-f1_N, py-f1_N), tex2D(Minv_yx, px, py-f1_N), tex2D(Minv_yx, px+f1_N, py-f1_N), tex2D(Minv_yx, px+f2_N,py-f1_N)),
                     cubicFilter(fx, tex2D(Minv_yx, px-f1_N, py),      tex2D(Minv_yx, px, py),      tex2D(Minv_yx, px+f1_N, py),      tex2D(Minv_yx, px+f2_N, py)),
                     cubicFilter(fx, tex2D(Minv_yx, px-f1_N, py+f1_N), tex2D(Minv_yx, px, py+f1_N), tex2D(Minv_yx, px+f1_N, py+f1_N), tex2D(Minv_yx, px+f2_N, py+f1_N)),
                     cubicFilter(fx, tex2D(Minv_yx, px-f1_N, py+f2_N), tex2D(Minv_yx, px, py+f2_N), tex2D(Minv_yx, px+f1_N, py+f2_N), tex2D(Minv_yx, px+f2_N, py+f2_N))
                                                );
